        ...
        try:            
            ...
            for row in data:
                ...
                record = {'phNum': int(phoneNum),
                          ...
                          'groundTruth': {'label': row[10]}}

                if row[10] == 'Trip' and len(row) > 11:
                    record['groundTruth']['mode'] = row[11]
                    if row[11] == 'Transit' and len(row) > 12:
                        record['groundTruth']['transitAgency'] = row[12]
                        if row[12] == 'Other' and len(row) > 13:
                            record['groundTruth']['transitAgencyName'] = row[13]
                    elif row[11] == 'Other' and len(row) > 12:
                        record['groundTruth']['modeName'] = row[12]
                elif row[10] == 'Activity' and len(row) > 11:
                    record['groundTruth']['purpose'] = row[11]
                    if row[11] == 'Other' and len(row) > 12:
                        record['groundTruth']['purposeName'] = row[12]
                        if len(row) > 13:
                            record['groundTruth']['exactLocation'] = row[13]
                    elif len(row) > 12:
                        record['groundTruth']['exactLocation'] = row[12]
